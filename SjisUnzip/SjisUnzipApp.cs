...
namespace SjisUnzip
{
	public class SjisUnzipApp
	{
		...
		{
			"Usage: sjisunzip [-c <Codepage Number>] someFile.zip [toFolder]".wl();
			"    -c: Sets the codepage the extracter will use (default 932). Common ones are 932 (JP), 936 (CN), and 1252 (EN)".wl();
			"Usage: sjisunzip -r [-c <Codepage Number>] someFile.zip".wl();
            ...
            "    sjisunzip -c 936 aFile.zip".wl();
            "    sjisunzip -c 936 aFile.zip MyNewFolder".wl();
			"    sjisunzip -r aFile.zip".wl();
			"    sjisunzip -r -c 936 aFile.zip".wl();
		}
